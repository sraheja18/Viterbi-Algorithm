...
class Evaluator(object):
    ...
    def compare(self, gold_standard, prediction):
        ...
        for gs_word, gs_tag in gold_standard:  # Move through the gold standard stream
            pred_word, pred_tag = next(
                prediction
            )  # Get the corresponding item from the prediction stream

            # Make sure words in both files match up
            if gs_word != pred_word:
                sys.stderr.write(
                    "Could not align gold standard and predictions in line %i.\n"
                    % (total + 1)
                )
                sys.stderr.write(
                    "Gold standard: %s  Prediction file: %s\n" % (gs_word, pred_word)
                )
                sys.exit(1)
            ...
            pred_ends = curr_pred_type != None and (
                (pred_tag == None or pred_tag[0] in "OB")
                or (curr_pred_type != pred_type and pred_tag[0] == "I")
            )
            gs_ends = curr_gs_type != None and (
                (gs_tag == None or gs_tag[0] in "OB")
                or (curr_gs_type != gs_type and gs_tag[0] == "I")
            )
            ...
            if pred_word != None:
                pred_start = (
                    (pred_tag != None and pred_tag[0] == "B")
                    or (curr_pred_type == None and pred_tag[0] == "I")
                    or (
                        curr_pred_type != None
                        and curr_pred_type != pred_type
                        and pred_tag.startswith("I")
                    )
                )
                gs_starts = (
                    (gs_tag != None and gs_tag[0] == "B")
                    or (curr_gs_type == None and gs_tag[0] == "I")
                    or (
                        curr_gs_type != None
                        and curr_gs_type != gs_type
                        and gs_tag.startswith("I")
                    )
                )
            else:
                pred_start = False
            ...
            if (
                gs_ends and pred_ends
            ):  # GS and prediction contain a named entity that ends in the same place
                ...
                gs_ends
            ):  # Didn't find the named entity in the gold standard, count false negative
                self.fn += 1
                self.class_counts[curr_gs_type].fn += 1
            elif (
                pred_ends
            ):  # Named entity in the prediction doesn't match one int he gold_standard, count false positive
                self.fp += 1
                self.class_counts[curr_pred_type].fp += 1
            elif (
                curr_pred_type == None and curr_pred_type == None
            ):  # matching O tag or end of sentence, count true negative
                self.tn += 1
                for c in self.ne_classes:
    ...
    def print_scores(self):
        ...
        print(
            "Found %i GENEs. Expected %i GENEs; Correct: %i.\n"
        ...
        if (
            self.tp + self.fn == 0
        ):  # Prediction marked everything as a NE of the wrong type.
            rec = 1
        else:
        ...
        for c in self.ne_classes:
            ...
            if (c_tp + c_fn) == 0:
                sys.stderr.write(
                    "Warning: no instances for entity type %s in gold standard.\n" % c
                )
                c_rec = 1
            else:
            ...
            if (c_tp + c_fp) == 0:
                sys.stderr.write(
                    "Warning: prediction file does not contain any instances of entity type %s.\n"
                    % c
                )
                c_prec = 1
            else:

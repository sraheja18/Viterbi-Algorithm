...
class Hmm(object):
    ...
    def preprocess(self, corpus_file):
        """Preprocesses the input file to be fed into the model"""
        c = Counter()
    ...
    def read_counts(self, corpusfile):
        """Adds n-gram counts to dictionary"""
    ...
    def emissionprobability(self):
        ...
        for word, tag in self.emission_counts:
            self.e[(word, tag)] = self.emission_counts[(word, tag)] / (
                self.ngram_counts[0][(tag,)]
            )

    def transitionprobability(self, delta=0.3):
        """Calculates transition probability"""
        for x, y, z in self.ngram_counts[2]:
            self.q[(x, y, z)] = (self.ngram_counts[2][(x, y, z)] + delta) / (
                self.ngram_counts[1][(x, y)] + 3 * delta
            )
    ...
    def base_tagger(self, filename):
        """Evaluates the base tagger and add it to the file"""
    ...
    def viterbi(self, filename):
        ...
        f = open("gene_dev.viterbi_laplace", "w")
